...
describe("calcolo maggiorazioni dal cedolino", () => {
  ...
  it("mantiene paga giornaliera e mensile separate senza inventare quella oraria", () => {
    const result = analyzePayslipText("Retribuzione giornaliera | 82,50\nRetribuzione mensile | 1.780,00");
    expect(result.basePay.value).toBeNull();
    expect(result.dailyPay.value).toBe(82.5);
    expect(result.monthlyPay.value).toBe(1780);
  });
